...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법4
        BackThread4 thread4 = new BackThread4(handler4);  // 메인 스레드의 Handler 객체를 외부 클래스에 넘겨줌
   ...
   class BackThread1 extends Thread{
       ...
       public void run() {
           while(true){
               ...
               // 방법2) 메인에서 생성된 Handler 객체의 postXXX()를 통해 Runable 객체 전달
               backValue2+=2;
               handler2.post(new Runnable() {
                   @Override
                   public void run() { // Runable 의 run()에서 메인 UI 접근
    ...
    public void handleMessage(@NonNull Message msg) {
        if(msg.what == 1){ // Message id가 1이면
            tvBackValue1.setText("BackValue1:"+backValue1); // 메인스레드 UI 변경
        }
    }
};
...
class BackThread3 extends Thread {
    ...
    public void run() {
        while (true){
            ...
            handler.sendMessage(msg); // 메인스래드의 핸들러에 메세지 보내기
