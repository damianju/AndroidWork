...
public class Main4Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        helper = new MySQLiteOpenHelper4(this, dbName, null, dbVersion);

        try{
            db = helper.getWritableDatabase();
        } catch (SQLException e) {
            e.printStackTrace();
            Log.e("myapp", "데이터베이스를 열 수 없음"); // Logcat 의 error 에 표시됨
            }
    ...
    void update(String name, int age, String address){
        ...
        // 리턴값은 affected rows
        int cnt = db.update(tableName, values, "name = ?", new String[]{name}); // 테이블 명, 변경할 값들, WHERE 조건절, 조건절의 ?값들
    ...
    void delete(String name){
        ...
        // 리턴값은 affected rows
        int cnt = db.delete(tableName, "name=?", new String[]{name}); // 테이블명, WHERE 조건절, 위 조건절의 ?값들
