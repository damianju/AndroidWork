...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    File f = new File(path, "external.txt"); // 경로, 피일명
                    FileWriter writer = new FileWriter(f, false); // true 이면 append 모드 생성. false이면 write 모드.
        ...
        btnRead.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(!checkExternalStorage()) return; // 외부 메모리 사용 가능 여부 체크
    ...
    protected boolean checkExternalStorage(){
        ...
        // 외부 메모리 상태
        if(Environment.MEDIA_MOUNTED.equals(state)){ // "mounted" 읽기와 쓰기가 모두 가능한 상태
            msg = "외부메모리 읽기 쓰기 모두 가능";
            Log.d("myapp", msg);
            Toast.makeText(this,msg,Toast.LENGTH_LONG).show();
            return  true;
        } else if(Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)){ // "mounted_ro" 읽기 전용
            msg= "외부메모리 읽기만 가능";
            Log.d("myapp", msg);
            Toast.makeText(this,msg,Toast.LENGTH_LONG).show();
            return  false;
