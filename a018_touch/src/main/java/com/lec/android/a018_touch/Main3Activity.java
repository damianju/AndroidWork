...
public class Main3Activity extends AppCompatActivity {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(pointCount>3) pointCount = 3; //3개 이상의 포인트 터치했더라도 3개까지만 처리

        switch (event.getAction() & MotionEvent.ACTION_MASK){
            case MotionEvent.ACTION_DOWN: // 한개 포인트에 대해 DOWN이 발생했을 때
                ...
                result += "("+x[0]+", "+y[0]+")";
                break;
            case MotionEvent.ACTION_POINTER_DOWN: // 두 개 이상의 포인트에 대한 DOWN이 발생했을 떄
                result = "멀티터치 : \n";
