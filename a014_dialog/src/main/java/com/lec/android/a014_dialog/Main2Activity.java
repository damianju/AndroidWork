    ...
    protected AlertDialog.Builder showAlert(int id){
        switch (id){
            case DIALOG_TEXT:
                ...
                builder1.setTitle("다이얼로그 제목임").setMessage("안녕들 하세요~!").setPositiveButton("긍정", new DialogInterface.OnClickListener() {
                    @Override
            ...
            case DIALOG_LIST:
                ...
                builder2.setTitle("리스트 형식 다이얼로그 제목").setNegativeButton("취소", null).setItems(str, // 리스트 목록에 사용할 배열
                        new DialogInterface.OnClickListener() {
                    //리스트 아이템 선택되었을 때 호출되는 콜백
                    // which 몇 번째 선택되었는지 에 대한 값
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
            ...
            case DIALOG_RADIO:
                ...
                builder3.setTitle("색을 고르세요").setPositiveButton("선택 완료", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        Toast.makeText(getApplicationContext(), str2[choice] + "을 선택", Toast.LENGTH_LONG).show();
                    }
                }).setNegativeButton("취소", null).setSingleChoiceItems(str2, -1, new DialogInterface.OnClickListener() {
                   // 라디오 버튼에서 선택 했을 때 호출되는 콜백
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        choice = which;
                    }
            ...
            case DIALOG_MULTICHOICE:
                ...
                builder4.setTitle("MultiChoice 다이얼로그 제목").setPositiveButton("선택 완료", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        String str = "선택된 값은: ";
               ...
               }).setNegativeButton("취소", null).setMultiChoiceItems(data, checked, new DialogInterface.OnMultiChoiceClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which, boolean isChecked) {
                        checked[which] = isChecked;
                    }
                }) // 체크박스 리스트 항목, 초기값(선택 여부) 배열, 체크박스 선택 했을 때 호출되는 콜백
                ;
                builder4.show();
                return  builder4;
