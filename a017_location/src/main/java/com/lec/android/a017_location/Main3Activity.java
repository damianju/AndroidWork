    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnGeoCoder1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    list = geocoder.getFromLocation(lat, lng,10); // 위도, 경도, 얻어올 결과값의 최대 개수
                } catch (IOException e) {
                    e.printStackTrace();
                    Log.d("myapp", "입출력 오류 - 서버에서 주소 변환시 에러 발생");
                }

                if(list != null){
                    if(list.size() ==0){
                        tvResult.setText("해당되는 주소정보가 없습니다");
                    } else{
        ...
        btnGeoCoder2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    list = geocoder.getFromLocationName(str,10); // 지명 이름, 얻어올 결과값의 최대 개수
                } catch (IOException e) {
                    e.printStackTrace();
                    Log.d("myapp", "입출력 오류 - 서버에서 주소 변환시 에러 발생");
                }
                ...
                if(list != null){
                    if(list.size() ==0){
                        ...
                        for (Address addr: list){
                            result.append("---------------------------\n");
                            result.append(addr.getCountryName()+", "+addr.getFeatureName()+", "+addr.getLocality()+", "+addr.getLatitude()+", "+addr.getLongitude());
