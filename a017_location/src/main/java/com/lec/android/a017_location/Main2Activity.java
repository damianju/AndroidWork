...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            if (checkSelfPermission(String.valueOf(permissions)) == PackageManager.PERMISSION_GRANTED) {
        ...
        btnLocate.setOnClickListener(new View.OnClickListener() {
                 ...
                 public void onClick(View v) {
                     if(btnLocate.isChecked()){
                         ...
                         try {
                             // 주어진 위치제공자가 기억하는 가장 최근의 위치 정보 획득, 없으면 null 리턴
                             Location location = manager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
                             if(location != null){
                                 tvResult.setText("최근 위치\n"+locationInfo(location));
                             }
                             manager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 100, 1, mLocationListener); // 등록할 위치 제공자, 통지사이의 최소 시간간격(ms), 통지사이의 최소변경 거리(m), LocationListener
                             manager.requestLocationUpdates(LocationManager.NETWORK_PROVIDER, 100, 1, mLocationListener); // 등록할 위치 제공자, 통지사이의 최소 시간간격(ms), 통지사이의 최소변경 거리(m), LocationListener
    ...
    // LocationListener 구현
    // LocationManager로 부터 전달되는 위치 정보를 받기 위해 정의
        ...
        switch (requestCode) {
            case REQUEST_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(this, "위치 권한 획득", Toast.LENGTH_LONG).show();

                } else {
                    Toast.makeText(this, "위치 권한 획득 실패", Toast.LENGTH_LONG).show();
                }
        }
    }
}
