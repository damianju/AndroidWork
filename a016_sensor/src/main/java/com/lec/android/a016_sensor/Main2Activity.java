...
public class Main2Activity extends AppCompatActivity implements SensorEventListener {
    ...
    // TYPE_ORIENTATION <-- 현재 deprecated
    ...
    protected void onResume() {
        ...
        sm.registerListener((SensorEventListener) this, accelerometer, SensorManager.SENSOR_DELAY_UI); // this, 원하는 센서, 지연시간(2ms, 샘플 추출)
        sm.registerListener((SensorEventListener) this, magnetometer, SensorManager.SENSOR_DELAY_UI);
    }
    ...
    // 센서 값이 변경될 때마다 호출되는 콜백
    @Override
    public void onSensorChanged(SensorEvent event) {

        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) mGravity = event.values; // 센서값들은 float[] 로 넘어옴.
