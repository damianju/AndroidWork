...
public class Start extends AppCompatActivity implements View.OnClickListener {
                ...
                public void run() {
                    if (time >= 0) {
                        ...
                        if (time > 0) {
                            ...
                            builder.setTitle("타임아웃").setMessage("점수 : " + point).setNegativeButton("그만한기", new DialogInterface.OnClickListener() {
                                @Override
    ...
    public void onClick(View v) {
        ...
        switch (imageView.getId()) {
            // 맨 아래 블럭 imageview의 생사오가 일치하는 버튼인지 판정
            case R.id.ivRed: // 빨강버튼 클릭시
                if ("0".equals(iv[7].getTag().toString())) isOk = true; // 빨강블럭의 tag값은 "0"
                break;
            case R.id.ivGreen: // 초록버튼 클릭시
                if ("1".equals(iv[7].getTag().toString())) isOk = true; // 초록블럭의 tag값은 "1"
                break;
            case R.id.ivBlue: // 파랑록버튼 클릭시
                if ("2".equals(iv[7].getTag().toString())) isOk = true; // 파랑블럭의 tag값은 "2"
                break;
        } // end switch
        if (isOk) { // 버튼 색깔을 맞추었다면!
            ...
            int num = new Random().nextInt(3); // 0,1,2 증 랜덤
